...
void UpdateDevice(PDEV_BROADCAST_DEVICEINTERFACE pDevInf, WPARAM wParam)
{
	...
	if (((szClass.Find(_T("HID")) >=0) && (szDevId.Find(_T("VID_0483")) >= 0) && (szDevId.Find(_T("PID_5750")) >= 0)))
	{
		if (DBT_DEVICEARRIVAL == wParam)
		{
			CUsbHidDevice::getInstance()->OpenDevice();
		}
		else
		{
...
{
	// This thread is to listen USB arrival / remove events
	DWORD iThread;
	...
	CCube cube;
	// Register callback
	pDevice->RegisterRespCallback(&cube);
	pDevice->StartSensor();
	cube.main(argc, argv);

	pDevice->StopSensor();
	pDevice->UnregisterRespCallback(&cube);
	pDevice->CloseDevice();
